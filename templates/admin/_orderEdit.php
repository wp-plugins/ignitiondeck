...
<a href="admin.php?page=order_details"> &lt; Back to <?php echo $tr_Order_Details; ?> List</a>
		...
		<ul>
		<form action="" method="post" name="formEditOrder" id="formEditOrder">
			<li class="first">
			...
			<li>
				...
				<div>
					<select class="select" id="product_id" name="product_id" onchange="get_levels('<?php echo site_url(); ?>')">
							<?php
						foreach ($products as $product) {
							echo '<option '.($product->id == absint($order_data->product_id) ? 'selected' : '').' value="'.absint($product->id).'">'.stripslashes(html_entity_decode($product->product_name)).'</option>';
						}
						?>
					</select>
				</div>
			...
			<li id="manual-select" style="display: none;">
				<label><?php echo $tr_Manual_Amount; ?></label>
				<div><input type="text" class="textbox" name="manual-input" id="manual-input" /></div>
			</li>
			<li>
				<div>
					<a id="manual-switch" href="#"><?php echo $tr_Manual_Switch; ?></a>
				</div>
			</li>
			<li>
